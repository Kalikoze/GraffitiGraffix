    ...
    .then(
      user =>
        !user.length
          ? response
              .status(404)
              .json({ error: 'Google UID could not be found.', google_uid })
          : response.status(200).json(user[0])
    );
});
...
app.post('/api/v1/users', (request, response) => {
  ...
  const keys = ['name', 'username', 'tag', 'shortBio', 'google_uid'];
...
app.post('/api/v1/followers', (request, response) => {
  ...
  const keys = ['follower_id', 'artist_id'];
    ...
    .then(
      length =>
        length
          ...
          : response
              .status(404)
              .send({ error: `No user to delete with id of ${id.id}` })
    )
    .catch(error => response.status(500).json({ error }));
});
    ...
    .then(
      length =>
        length
          ...
          : response
              .status(404)
              .send({ error: `No image to delete with id of ${id.id}` })
    )
    .catch(error => response.status(500).json({ error }));
});
    ...
    .then(
      length =>
        length
          ...
          : response
              .status(404)
              .send({ error: `No comment to delete with id of ${id.id}` })
    )
    .catch(error => response.status(500).json({ error }));
});
  ...
  db('users')
    ...
    .then(update => {
      if (!update.length) {
        response
          .status(404)
          .json({ error: `Cannot find user with id of ${id.id}` });
      }
  ...
  db('comments')
    ...
    .then(update => {
      if (!update.length) {
        response
          .status(404)
          .json({ error: `Cannot find comment with id of ${id.id}` });
      }
      response.status(200).json(update[0]);
    })
    .catch(error => response.status(500).json({ error }));
